...
fn get_gassed_amount(row: i128, values: &[i128]) -> i128 {
    return values.iter()
        .map(|number| {
            return (row - number).abs();
        }).sum();
}

fn get_cost_map(max: i128) -> HashMap<i128, i128> {
    let mut map = HashMap::<i128, i128>::new();

    let mut sum = 0;
    for index in 0..=max {
        sum += index;
        map.insert(index.clone(), sum.clone());
    }

    return map;
}

fn get_gassed_squared_amount(row: i128, values: &[i128], cost_map: &HashMap<i128, i128>) -> i128 {
    return values.iter()
        .map(|number| {
            let steps = (row - *number).abs();
            let cost = cost_map.get(&steps).unwrap();
            return cost;
        }).sum();
}

fn puzzle1(input: &str) -> i128 {
    let number_values: Vec<i128> = input.split(',')
        .map(|str| str.parse::<i128>().unwrap())
        .collect();

    let min = *number_values.iter().min().unwrap();
    let max = *number_values.iter().max().unwrap();

    let mut least_gassed_amount = i128::MAX;
    for i in min + 1..=max {
        let current_gassed_amount = get_gassed_amount(i, &number_values);
        if current_gassed_amount < least_gassed_amount {
            least_gassed_amount = current_gassed_amount;
        }
    }
    return least_gassed_amount;
}

fn puzzle2(input: &str) -> i128 {
    let number_values: Vec<i128> = input.split(',')
        .map(|str| str.parse::<i128>().unwrap())
        .collect();
    ...
    let cost_map = get_cost_map(max);

    let mut least_gassed_amount = i128::MAX;
    for i in min + 1..=max {
        let current_gassed_amount = get_gassed_squared_amount(i, &number_values, &cost_map);
        if current_gassed_amount < least_gassed_amount {
            least_gassed_amount = current_gassed_amount;
        }
    }
